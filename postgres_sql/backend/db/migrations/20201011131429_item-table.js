...
exports.up = async (knex) => {
    await knex.table(tableNames.state, (table) => {
        table.string('code');
    });
    await knex.table(tableNames.country, (table) => {
        table.string('code');
    });
...
exports.down = async (knex) => {
    ...
    ].reverse().map(name => knex.schema.dropTable(name)));
    await knex.schema.dropTableIfExists(tableNames.size);
};
